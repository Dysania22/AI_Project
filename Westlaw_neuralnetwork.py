...
# Find the top 10 closest cases to a search term
def find_closest_cases_for_search_term(search_term, model, doc_vectors, data, top_n=10):
    """Find the top 10 closest cases based on a search term and display in a pretty table."""

    case_names = [doc['case_name'] for doc in data]  # Get the list of case names

    # Infer vector for the search term
    search_vector = model.infer_vector(preprocess_text(search_term))

    # Compute cosine similarities between the search term vector and all document vectors
    similarities = cosine_similarity([search_vector], doc_vectors)[0]

    # Get the indices of the top 10 most similar cases
...
if __name__ == "__main__":
    ...
    # Allow user input to choose between searching by case name or search term
    while True:
        user_choice = input("\nDo you want to search by 'case' or 'term'? (or type 'exit' to quit): ").strip().lower()
        ...
        if user_choice == 'case':
            user_input = input("Enter a case name to find the 10 closest cases: ").strip()
            ...
            find_closest_cases_for_search_term(search_term, model, doc_vectors, data)

        else:
            print("Invalid option. Please type 'case' or 'term'.")